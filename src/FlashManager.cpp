...
bool TrySaveLoraConfiguration(const LoraConfiguration& config) {
    ...
    ERR_CHECK(flashMemory.putLong(LORA_FREQUENCY_KEY, config.frequency));
    ERR_CHECK(flashMemory.putLong(LORA_BANDWIDTH_KEY, config.bandwidth));
    ERR_CHECK(flashMemory.putInt(LORA_SPREADING_FACTOR_KEY, config.spreadingFactor));
    ERR_CHECK(flashMemory.putInt(LORA_CODING_RATE_KEY, config.codingRate));
    ERR_CHECK(flashMemory.putInt(LORA_SYNC_WORD_KEY, config.syncWord));
    ERR_CHECK(flashMemory.putInt(LORA_POWER_KEY, config.power));
    ERR_CHECK(flashMemory.putBool(LORA_CRC_ENABLED_KEY, config.crcEnabled));
    flashMemory.end();
    return true;
}
