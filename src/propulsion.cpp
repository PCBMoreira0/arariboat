...
static void get_velocity_in_bytes(uint8_t& bb_vel, uint8_t& be_vel, float bb_pot, float br_pot, float dir_pot){
    ...
    else
    {
        ...
        {
            uint8_t be_value = 255 * use_function(map(dir_pot, pot_dir_max_voltage, dir_central_voltage + dead_zone_threshold, 0, pot_dir_max_voltage) / pot_dir_max_voltage);
            bb_vel = 255 * speed_porc;
...
void propulsion_task(void* parameter) {
    ...
    for (;;) {
        ...
        get_velocity_in_bytes(bb_vel, be_vel, bb_pot, be_pot, dir_pot);
